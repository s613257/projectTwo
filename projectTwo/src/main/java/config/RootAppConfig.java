...
@Configuration
@EnableWebMvc
@ComponentScan(basePackages = {"config","controller","db","model","service","test"})
...
public class RootAppConfig {
	...
	public static DataSource dataSource() throws IllegalArgumentException, NamingException {
		...
		ds.setMaxTotal(100); // 最大連線數 以前叫做 maxActive。設定為 10 表示最多能有 10 人同時連線；設定為 0 表示無上限
		ds.setMaxIdle(100); // 最大空閒時間，若是超過時間，連線將被視為不可用。設定為 0 的話表示無上限
	...
	public static LocalSessionFactoryBean sessionFactory() throws IllegalArgumentException, NamingException {
		...
		factory.setPackagesToScan(new String[] { "config","controller","db","model","service","test"});
